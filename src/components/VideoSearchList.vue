<template>
  <el-row class="mov-card-row"
    ...
    <el-col v-for="item in videoList"
            ...
            :md="4"
            class="mov-card">
      <router-link :to="'/VideoDetailPage/'+ encodeURIComponent(JSON.stringify(item))"
  ...
  <el-backtop :right="50"
              :bottom="80" />
  <p v-show="infiniteMsgShow"
     class="tips">Loading...</p>
  <p v-show="!infiniteMsgShow"
     class="tips">到底啦</p>
</template>
...
import { getPageVideoBySearch } from '@/apis/video'
...
export default {
  ...
  setup (props) {
    ...
    const getVideoList = () => {
      ...
      getPageVideoBySearch(data).then(
        (res) => {
